...
export default function ApplePredictor({ onBack, accessKeyData, language, onLanguageChange, platform }: ApplePredictorProps) {
  ...
  const handleGeneratePrediction = async () => {
    ...
    // Fetch predictions from Firebase RTDB (do NOT overwrite an already populated DB!)
    let liveGrid: boolean[][] | null = null;
    try {
      liveGrid = await fetchPredictionsFromFirebase();
      
      // If liveGrid is null or contains all false values (empty DB), seed it!
      const isGridEmpty = !liveGrid || liveGrid.every((row) => row.every((isSafe) => !isSafe));
      if (isGridEmpty) {
        console.log("Database was empty or corrupted. Seeding standard starting grid...");
        await uploadPredictionsToFirebase();
        liveGrid = await fetchPredictionsFromFirebase();
      }
    } catch (e) {
      console.error("Failed to fetch predictions from Firebase RTDB:", e);
    }
      ...
      for (let r = 0; r < rowCount; r++) {
        if (r < liveGrid.length) {
          ...
          if (safeColIndices.length > 0) {
            ...
            // No safe column found on this row -> fallback to a random column and make sure it is safe!
            const fallbackCol = Math.floor(Math.random() * 5);
            liveGrid[r][fallbackCol] = true;
            calculatedPath.push(fallbackCol);
          }
        } else {
